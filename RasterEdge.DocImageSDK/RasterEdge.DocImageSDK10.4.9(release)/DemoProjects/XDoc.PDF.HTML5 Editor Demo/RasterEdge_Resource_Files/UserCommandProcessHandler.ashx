...
public class UserCommandProcessHandler : RasterEdge.XDoc.PDF.HTML5Editor.ProcessHandler
{
    public override void FileProcess()
    {
        ...
        if (command == null || command.Equals(""))
        {
            ...
            else if (!string.IsNullOrEmpty(request.QueryString["yourtarget"]))
            {
                ...
                // RasterEdge.XDoc.PDF.PDFDocument pdf = null;// your object
                RasterEdge.Imaging.Basic.BaseDocument pdf = new RasterEdge.XDoc.PDF.PDFDocument(@"d:\\1.pdf");
                webDoc = REProcessControl.PageLoadFile(request, pdf, "yourdoc.pdf");// object can be PDFDocument,File bytes or File Stream
            }
            else
            {
    ...
    public void SaveFileOnServer()
    {
        ...
        if (document !=null)
        {
            ...
            string paraFilepathValue = docinfo.GetRequestParameters("filepath");
